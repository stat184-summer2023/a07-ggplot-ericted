...
ggplot(Trips_filtered, aes(x = client, y = duration/3600)) +
  geom_boxplot() +
  labs(x = "Client Type", y = "Duration (hours)", title = "Distribution of Rental Duration by Client Type")
```
...
# Extract day of the year from sdate
day_of_year <- yday(Trips$sdate)

# Create histogram
ggplot(data = data.frame(day_of_year), aes(x = day_of_year)) +
  geom_histogram(fill = "skyblue", color = "black", bins = 30) +
  labs(x = "Day of Year", y = "Frequency", title = "Bike Use by Day of Year")


```


From 275 to 310, the frequency of usage is similar, but after that, it gradually decreases.
...
day_of_week <- wday(Trips$sdate, label = TRUE)


ggplot(data = data.frame(day_of_week), aes(x = day_of_week)) +
  geom_bar(fill = "skyblue", color = "black") +
  labs(x = "Day of Week", y = "Frequency", title = "Bike Use by Day of Week") +
...
There is no significant variation throughout the days of the week (Monday, Tuesday, Wednesday, Thursday, Friday, Saturday, Sunday); they are all fairly similar.
...
hour_of_day <- hour(Trips$sdate)
ggplot(data = data.frame(hour_of_day), aes(x = hour_of_day)) +
  geom_histogram(binwidth = 1, fill = "skyblue", color = "black") +
  labs(x = "Hour of Day", y = "Frequency", title = "Bike Use by Hour of Day") +
...
Between 8 AM and 9 AM, as well as between 6 PM and 7 PM, the highest number of people utilized the service. On the other hand, people hardly used the service from 12 AM to 7 AM. For the remaining hours, the usage is relatively consistent on average.
...
minute_in_hour <- minute(Trips$sdate)


ggplot(data = data.frame(minute_in_hour), aes(x = minute_in_hour)) +
  geom_histogram(fill = "skyblue", color = "black", bins = 30) +
  labs(x = "Minute in Hour", y = "Frequency", title = "Bike Use by Minute in Hour")


```


I did not find any significant differences when grouping the bike rentals by minute.
...
grouped_data <- Trips %>%
  group_by(hour = lubridate::hour(sdate), wday = lubridate::wday(sdate), client) %>%
...
ggplot(grouped_data, aes(x = hour, y = count)) +
  ...
  labs(x = "Hour of the Day", y = "Count of Events", title = "Bike Rentals by Hour of the Day, Day of the Week, and Client Type")
...
During weekdays, the highest number of bike rentals was observed during the morning and evening commute hours. Graphs 1 and 7 show that the peak usage occurred around 10 AM, followed by a gradual decrease throughout the day.
...
RideDistances <- Trips %>% inner_join(PairDistances, by = c("sstation", "estation"))


 ggplot(RideDistances, aes(x = distance, fill = client)) +
  geom_density(alpha = 0.5) +
  labs(x = "Distance (km)", y = "Density", fill = "Client Type") +
  theme_minimal()
...
The variables Simple and Simple2 have been assigned different names in order to differentiate between the start and end stations in the Trips dataset. Simple denotes the name of the station where the bicycle was checked out, while Simple2 represents the name of the station where the bicycle was returned. This naming convention facilitates easier differentiation and analysis of the information pertaining to the start and end stations.
